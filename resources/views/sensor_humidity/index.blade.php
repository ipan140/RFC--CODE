...
@section('content')
    ...
    <section class="section">
        @if(isset($data['humidity']) && !empty($data['humidity']))
            ...
            <div class="card shadow-sm mb-4">
                <div class="card-body">
                    <h5 class="card-title mb-3 text-dark fw-semibold">
                        <i class="bi bi-graph-up me-2"></i>Sensor Humidity
                    </h5>

                    <div class="mb-3 d-flex gap-2">
                        <a href="{{ url('/export/humidity') }}" class="btn btn-success">
                        ...
                        @if(in_array($userRole, ['admin', 'owner']))
                            <button type="button" class="btn btn-primary" id="btn-fetch-humidity">
                                ...
                                <tr>
                                    ...
                                    <td class="d-flex justify-content-center gap-1">
                                        ...
                                        @if(in_array($userRole, ['admin', 'owner']))
                                            ...
                                            <form action="{{ route('sensor_humidity.destroy', $sensorData['ri']) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data?')">
                                                @csrf
                                                @method('DELETE')
            ...
            <div class="modal fade" id="modalTambah" tabindex="-1" aria-labelledby="modalTambahLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <form action="{{ route('sensor_humidity.store') }}" method="POST" class="modal-content">
                        @csrf
                        <div class="modal-header">
                            <h5 class="modal-title">Tambah Data Humidity</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                        </div>
                        <div class="modal-body">
                            <div class="mb-3">
                                <label class="form-label">Nilai Humidity</label>
            ...
            <div class="modal fade" id="modalEdit" tabindex="-1" aria-labelledby="modalEditLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <form action="{{ route('sensor_humidity.update', $sensorData['ri']) }}" method="POST" class="modal-content">
                        @csrf
                        @method('PUT')
                        <div class="modal-header">
                            <h5 class="modal-title">Edit Data Humidity</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                        </div>
                        <div class="modal-body">
                            <div class="mb-3">
            ...
            <div class="modal fade" id="modalView" tabindex="-1" aria-labelledby="modalViewLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title">Detail Data Humidity</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                        </div>
                        <div class="modal-body">
        ...
        @else
            <div class="alert alert-warning text-center">
                Data sensor Humidity tidak ditemukan.
            </div>
        @endif
    </section>
    </main>
    ...
    <script>
        document.getElementById('btn-fetch-humidity').addEventListener('click', function () {
            fetch("{{ route('sensor_humidity.fetch-store') }}")
